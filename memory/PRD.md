...
- **AI/live search:** provider-switchable via `LLM_PROVIDER` — Gemini (`google-genai` SDK + Google Search grounding, `GEMINI_API_KEY`; default, free tier for testing) or Anthropic (`anthropic` SDK + hosted `web_search_20260209` tool, `ANTHROPIC_API_KEY`; production). Returns structured JSON per a strict schema with guardrail fallback.
